...
class Query(object, metaclass=MetaQuery):
    ...
    def __new__(cls, *a, **kw):
        ...
        try:
            query_attr = query.apply if hasattr(query, 'apply') else query.get_state
            setattr(query, '__call__', query_attr)
        except AttributeError:
            raise MissingApplyMethod(
                f'Query {name} is missing a `get_state` method. All queries must '
                'include an `get_state` method which returns state for the records'
                'to resolve.'
            )
